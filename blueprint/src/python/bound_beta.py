...
def add_beta_bound(hypotheses, bounds, ref):
    for b in bounds:
        ...
        hypotheses.add_hypothesis(Hypothesis(f'{author} ({year}) bound on \\beta on {b.domain}', 
            'Upper bound on beta', Bound_beta(b), f'See [{author}, {year}]', ref))

def derived_bound_beta(bound, proof, dependencies):
    year = Reference.max_year(tuple(d.reference for d in dependencies))
    bound = Hypothesis('Derived bound on \\beta', 'Upper bound on beta', Bound_beta(bound), proof, Reference.derived(year))
...
def classical_bound_beta(bound):
    return Hypothesis( f'Classical bound on \\beta', 'Upper bound on beta', bound, f'Classical', Reference.classical())

trivial_beta_bound_1 = classical_bound_beta(Affine(1, 0, Interval(0, 1, True, True)))
trivial_beta_bound_2 = classical_bound_beta(Affine(1, -1, Interval(1, Constants.ALPHA_UPPER_LIMIT, False, True)))
...
def exponent_pairs_to_beta_bounds(hypothesis_set):
    ...
    for eph in ephs:
        k = eph.data.k
        l = eph.data.l
        hypotheses.append(derived_bound_beta(Affine(l - k, k, domain), 
            f'Follows from the exponent pair {eph.data}.', {eph}))
        
        # Sargos (1995) Theorem 7.1 - 
        # TODO: add this to the literature
        # TODO: include an exponent pair transform as a literature hypothesis
        pieces = Affine((6 * k + 5 * l + 2) / (2 * (5 * k + 3 * l + 2)), 
                        (5 * k + l + 2) / (8 * (5 * k + 3 * l + 2)), domain)\
                .max_with([Affine(frac(2,3), frac(1,12), domain)])
        
        for p in pieces:
            hypotheses.append(derived_bound_beta(p,
                      f'Follows from Sargos (1995) and the exponent pair {eph.data}', {eph}))
...
def compute_best_beta_bounds(hypothesis_set, domain=None):
    ...
    for b in best_bounds:
        ...
        if b == depend.data.bound:  
            ...
            derived_bounds.append(derived_bound_beta(b, f'Follows from {depend}', {depend}))
...
def display_beta_bounds(hypotheses):
    ...
    for h in hypotheses:
        p = h.data.bound
        plt.plot((p.domain.x0, p.domain.x1), 
                 (p.at(p.domain.x0, True), p.at(p.domain.x1, True)), color='black')
    plt.xlim((-0.05, 0.55))
    plt.ylim((-0.05, 0.5))
    plt.xlabel("α")
    plt.ylabel("β(α)")
