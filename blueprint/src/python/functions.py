...
class Interval:
    ...
    def parse(s):
        inner = s[1:-1].split(',')
        return Interval(frac(inner[0].strip()), frac(inner[1].strip()), s[0]=='[', s[-1]==']')
...
class Affine:
    ...
    def __eq__(self, other):
        if isinstance(other, Affine):
            ...
            if self.domain.length() == 0 and other.domain.length() == 0 and \
                self.domain.x0 == other.domain.x0:
    ...
    def compare_with(self, f2, domain, comparator):
        ...
        while j < len(pieces) - 1:
            ...
            p1 = pieces[j]
            p2 = pieces[j + 1]
            match = p1.domain.x1 == p2.domain.x0 and \
                    (p1.domain.include_upper or p2.domain.include_lower) and \
                    pieces[j].label == pieces[j + 1].label and \
                    ((p1.domain.length() == 0 and p1.at(p1.domain.x1) == p2.at(p2.domain.x0)) or
                     (p2.domain.length() == 0 and p1.at(p1.domain.x1) == p2.at(p2.domain.x0)) or
                     (p1.function_equals(p2)))
...
class Affine2:
    ...
    def to_string(coeffs, symbols):
        if symbols is None: raise ValueError('symbols')
        if len(symbols) < len(coeffs) - 1: raise ValueError('Not enough symbols')
    ...
    def scale(self, i, factor, additional_constraints):
        return Affine2(self.a.copy(), self.domain.scale(i, factor, additional_constraints))
    ...
    def min_with(self, other):
        ...
        for f in other:
            ...
            if domain.intersects(plane):
                ...
                d1 = domain.intersect(Polytope([[self.a[i] - f.a[i] for i in range(len(self.a))]]))
                d2 = domain.intersect(Polytope([[f.a[i] - self.a[i] for i in range(len(self.a))]]))
...
class Piecewise:
    ...
    def scale(self, i, factor, additional_constraints):
        return Piecewise([p.scale(i, factor, additional_constraints) for p in self.pieces])
    ...
    def simplify(self):
        ...
        def iteration(matchable, n):
            ...
            for key in matchable:
                ...
                group = matchable[key]
                for c in itertools.combinations(group, n):
                    union = Polytope.try_union([f.domain for f in c])
...
class SympyHelper:
    ...
    def to_frac(expr):
        ...
        if isinstance(expr, sympy.core.numbers.Rational):
            ...
            if isinstance(n, sympy.core.numbers.Integer) and \
                isinstance(n, sympy.core.numbers.Integer):
...
class RationalFunction:
    ...
    def _compute_critical_pts(self, interval):
        ...
        if not df.is_constant():
            crit.extend(SympyHelper.to_frac(r) for r in sympy.real_roots(df) 
    ...
    def intersections(self, other, interval):
        ...
        if not f.is_constant():
            return [SympyHelper.to_frac(r) for r in sympy.real_roots(f) if interval.contains(r)]
        return []
