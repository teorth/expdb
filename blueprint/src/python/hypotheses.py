...
class Hypothesis:
    ...
    def recursively_list_proofs(self, indentation=0):
        if len(self.dependencies) > 0:
            print("\t" * indentation + f'- [{self}]  i.e. {self.data}. {self.proof}. Dependencies:')
    ...
    def is_match(self, hypothesis_type = "Any", year = "Any"):
        if hypothesis_type == "Any" or hypothesis_type == self.hypothesis_type:
            if year == "Any" or self.reference.label == "Derived" or self.reference.label == "Classical":
                return True
            if self.reference.label == "Conjectured":
                return False
            if self.reference.year() != 'Unknown date' and self.reference.year() <= year:
                return True
        return False
...
class Hypothesis_Set:
    ...
    def list_hypotheses(self, hypothesis_type="Any", year="Any"):
        return [hypothesis for hypothesis in self if hypothesis.is_match(hypothesis_type, year)]

    # find a hypothesis in the set that matches the specified requirements
    def find_hypothesis(self, hypothesis_type="Any", data="Any", keywords="Any", year="Any"):
        for hypothesis in self:
            if hypothesis_type == "Any" or hypothesis.hypothesis_type == hypothesis_type:
                if data == "Any" or hypothesis.data == data:
                    if keywords == "Any" or all(k.strip() in hypothesis.name for k in keywords.split(',')):
