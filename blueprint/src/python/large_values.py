...
def literature_bound_LV_max(bounds, ref, params=''):
    ...
    return Hypothesis(f'{ref.author()} large value estimate' + params, 'Large value estimate', 
            Large_Value_Estimate(piecewise), f'See [{ref.author()}, {ref.year()}]', ref)

def derived_bound_LV(bound, proof, deps):
    year = Reference.max_year(tuple(d.reference for d in deps))
    bound = Hypothesis('Derived large value estimate', 'Large value estimate', 
                       Large_Value_Estimate(bound), proof, Reference.derived(year))
    bound.dependencies = deps
    return bound

def classical_LV_estimate(bounds):
    return Hypothesis('Classical large value estimate', 'Large value estimate', 
            Large_Value_Estimate(max_of(bounds)), 'Classical', Reference.classical())

def conjectured_LV_estimate(bounds, name):
    return Hypothesis(name, 'Large value estimate', max_of(bounds), f'Conjecture', Reference.conjectured())
...
def raise_to_power_hypothesis(k):
    ...
    def transform(hypothesis):
        ...
        new_bound = hypothesis.data.bound.scale(2, k, [[Constants.TAU_UPPER_LIMIT, 0, -1]])
        for piece in new_bound.pieces:
            piece.a = [k * ai for ai in piece.a]
            piece.a[2] /= k
        return derived_bound_LV(new_bound, f'Follows from raising {hypothesis} to the k = {k} power', {hypothesis})
        
    return Hypothesis(f'Raising to a power large value estimate transform with k = {k}', 'Large value estimate transform', 
        Large_Value_Estimate_Transform(transform), 'Classical', Reference.classical())
...
def piecewise_min(estimates, domain, constructor):
    ...
    if best_est is not None:
        for e in best_est.pieces:
            ...
            hyps.append(constructor(Piecewise([e]), f'Follows from {parent.name}', {parent}))
...
def best_large_value_estimate(hypotheses, domain=None):
    ...
    lv_transforms = hypotheses.list_hypotheses(hypothesis_type='Large value estimate transform')
