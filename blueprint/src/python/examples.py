...
def mu_bound_examples():
    ...
    print(f'The bound {bound} was obtained in {literature.find_hypothesis(data=bound).desc_with_proof()}')                                                     
    ...
    print()
    print(f'Now assume all unconditional exponent pairs, and all unconditional exponent pair transforms.')
    ...
    print()
    print('Under the same assumptions, the best piecewise-linear bound on mu in [1/3,9/10) is')
...
def exp_pair_examples():
    hypotheses = Hypothesis_Set()
    print(f'Assume all known exponent pairs as well as the van der Corput A/B transforms')
    ...
    print(f'The computed convex hull has the following {len(hyps)} exponent pairs as vertices.')
    for h in hyps:
        h.recursively_list_proofs()
        
        
    print()
    print(f'Proof that (3/40, 31/40) is an exponent pair, found by searching for the earliest proof')
    eph = ep.find_best_proof(frac(3,40), frac(31,40), hypotheses, Proof_Optimization_Method.DATE)
...
def beta_bound_examples():
    ...
    print()
    print('3. Now assume all exponent pairs in the literature, and the trivial pair (0, 1):')
    ...
    print('5. Recursive list of hypotheses used to derive the (1101653/15854002, 12327829/15854002) exponent pair')
...
def large_values_examples():
    ...
    hypotheses.add_hypothesis(lv.large_value_estimate_L2) # also add classical estimates
    ...
    transformed_LV_estimate = raise_to_second_power.data.transform(lv.large_value_estimate_L2)
...
def zeta_large_values_examples():
    hypotheses = Hypothesis_Set() # Start with an empty hypothesis set 
    hypotheses.add_hypotheses(literature.list_hypotheses(hypothesis_type='Upper bound on beta'))
    hypotheses.add_hypotheses(literature.list_hypotheses(hypothesis_type='Upper bound on mu'))
    hypotheses.add_hypotheses(literature.list_hypotheses(hypothesis_type='Large value estimate'))
...
def zero_density_estimates_examples():
    ...
    hypotheses.add_hypothesis(lv.large_value_estimate_L2)
    hypotheses.add_hypotheses(literature.list_hypotheses(hypothesis_type='Large value estimate'))
    for k in range(2, 15): hypotheses.add_hypothesis(lv.raise_to_power_hypothesis(k))
    
    hypotheses.add_hypotheses(zlv.get_trivial_zlv()) # Include the tribial zeta large value estimate
    hypotheses.add_hypotheses(literature.list_hypotheses(hypothesis_type='Zeta large value estimate'))
    ...
    for s in zdt: 
        ...
        #     for h in hyps:
        #         h.recursively_list_proofs(2)
    plot(zdt, literature.list_hypotheses(hypothesis_type='Zero density estimate'), 
...
def zero_density_estimates_examples2():
    ...
    # Zeta large value estimates
    hypotheses.add_hypothesis(literature.find_hypothesis(hypothesis_type='Zeta large value estimate', keywords='Heath-Brown'))
