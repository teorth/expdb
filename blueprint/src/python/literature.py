...
def add_beta_bound_watt_1989():
    bbeta.add_beta_bound(literature, [Affine(frac(1,2), frac(89,560), Interval('[3/7, 1/2]'))], 
...
def add_beta_bound_huxley_kolesnik_1991():
    bbeta.add_beta_bound(literature, 
        Affine(frac(8,22), frac(1,22), Interval('[2/5, 1/2]'))
            .max_with([Affine(frac(112,158), frac(11,158), Interval('[2/5, 1/2]')),
...
def add_beta_bound_huxley_1993():
    bbeta.add_beta_bound(literature, 
            Affine(frac(7,20), frac(13,60), Interval('[0, 49/144]'))
                .max_with([Affine(frac(13,20), frac(11,120), Interval('[0, 49/144]'))]), 
            rm.refs['huxley_exponential_1993'])
    bbeta.add_beta_bound(literature, [Affine(frac(1,2), frac(89,570), Interval('[49/144, 1/2]'))], 
                     rm.refs['huxley_exponential_1993'])
...
def add_beta_bound_huxley_1996_2():
    bbeta.add_beta_bound(literature, [
            ...
            Affine(frac(2484,6410), frac(1273,6410), Interval('[68682/171139, 307/761)')),
            Affine(frac(1053,2800), frac(569,2800), Interval('[307/761, 143/349)')),
            Affine(frac(3624,5530), frac(491,5530), Interval('[143/349, 263/638)')),
            Affine(frac(897,1345), frac(113,1345), Interval('[263/638, 156527/370694)')),
            Affine(frac(88442,134680), frac(11897,134680), Interval('[156527/370694, 699371/1647930)')),
            Affine(frac(19177,29855), frac(2819,29855), Interval('[699371/1647930, 675/1574)')),
            Affine(frac(17972,27290), frac(2387,27290), Interval('[675/1574, 106822/246639)')),
            Affine(frac(285,570), frac(89,570), Interval('[106822/246639, 139817/246639]'))
            ], 
        rm.refs['huxley_area_1996'])
...
def add_beta_bound_huxley_kolesnik_2001(): 
    bbeta.add_beta_bound(literature, 
            Affine(frac(79,120), frac(7,80), Interval('[2/5, 1/2]'))
                .max_with([Affine(frac(103,160), frac(3,32), Interval('[2/5, 1/2]')),
...
def add_beta_bound_robert_sargos_2002():
    bbeta.add_beta_bound(literature, 
            Affine(frac(9,13), frac(1,13), Interval('[0, 1]'))
                .max_with([Affine(frac(28,13), frac(-7,13), Interval('[0, 1]'))]),
...
def add_beta_bound_sargos_2003():
    bbeta.add_beta_bound(literature, 
            Affine(frac(204-8,204), frac(1,204), Interval('[0, 1]'))
                .max_with([Affine(frac(95*8,204), frac(-95,204), Interval('[0, 1]'))]),
            rm.refs['sargos_analog_2003'])

    bbeta.add_beta_bound(literature, 
            Affine(frac(2640-7*9,2640), frac(7,2640), Interval('[0, 1]'))
                .max_with([Affine(frac(1001*9,2640), frac(-1001,2640), Interval('[0, 1]'))]),
            rm.refs['sargos_analog_2003'])
...
def add_beta_bound_huxley_2005():
    bbeta.add_beta_bound(literature, 
            Affine(frac(59,170), frac(37,170), Interval('[1/3, 1/2]'))
                .max_with([Affine(frac(449,690), frac(21,230), Interval('[1/3, 1/2]'))]),
...
def add_beta_bound_robert_2016():
    bbeta.add_beta_bound(literature, 
            Affine(frac(2,3), frac(1,12), Interval('[0, 3/7]'))
                .max_with([Affine(frac(11,12), 0, Interval('[0, 3/7]'))]), 
...
def add_beta_bound_robert_2016_2(K):
    for k in range(4, K):
        bbeta.add_beta_bound(literature, 
                Affine(1 - frac(k,2*(k-1)*(k-2)), frac(1,2*(k-1)*(k-2)), Interval(0, frac(k-1, k*(k-1)-(2*k-3)), True, True))
                    .max_with([Affine(1 - frac(1,2*(k-1)*(k-2)), 0, Interval(0, frac(k-1, k*(k-1)-(2*k-3)), True, True))]), 
                rm.refs['robert_2016'])
...
def add_beta_bound_heath_brown_2017(K):
    for k in range(3, K):
        bbeta.add_beta_bound(literature, 
                Affine(frac(k-2, k-1), frac(1, k*(k-1)), Interval('[0, 1]'))
                    .max_with([Affine(1 - frac(1, k*(k-1)), 0, Interval('[0, 1]')),
...
def add_literature_exponent_pairs():
    literature.add_hypotheses([
        ...
        ep.literature_exp_pair(frac(9,56), frac(37,56), rm.get('huxley_exponential_1988')),
        ep.literature_exp_pair(frac(89,560), frac(369,560), rm.get('watt_exponential_1989')),
        ep.literature_exp_pair(frac(17,108), frac(71,108), rm.get('huxley_exponential_1991')),
        ep.literature_exp_pair(frac(89,570), frac(187,285), rm.get('huxley_exponential_1993')),
        ep.literature_exp_pair(frac(32,205), frac(269,410), rm.get('huxley_exponential_2005')),
        ep.literature_exp_pair(frac(13,84), frac(55,84), rm.get('bourgain_decoupling_2017')),
    
        # Exponent pairs (k, l) with l > k + 1/2. 
    
        # Exponent pair derived from Bombieri-Iwaniec method 
        ep.literature_exp_pair(frac(2,13), frac(35,52), rm.get('huxley_watt_exponential_1990')),
        ep.literature_exp_pair(frac(6299,43860), frac(29507,43860), rm.get('huxley_area_1996')),
        ep.literature_exp_pair(frac(771,8116), frac(1499,2029), rm.get('sargos_points_1995')),
        ep.literature_exp_pair(frac(21,232), frac(173,232), rm.get('sargos_points_1995')),
        ep.literature_exp_pair(frac(1959,21656), frac(16135,21656), rm.get('sargos_points_1995')),
        ep.literature_exp_pair(frac(516247,6629696), frac(5080955,6629696), rm.get('huxley_exponential_2001')),
    
        # Exponent pairs derived from k-th derivative tests
        ep.literature_exp_pair(frac(1,13), 1 - frac(3,13), rm.get('robert_fourth_2002')),
        ep.literature_exp_pair(frac(1,204), 1 - frac(7,204), rm.get('sargos_analog_2003')),
        ep.literature_exp_pair(frac(1,360), 1 - frac(8,360), rm.get('robert_2002')),
        ep.literature_exp_pair(frac(7,2640), 1 - frac(56,2640), rm.get('sargos_analog_2003')),
        ep.literature_exp_pair(frac(1,716), 1 - frac(9,716), rm.get('sargos_analog_2003')),
        ep.literature_exp_pair(frac(1,649), 1 - frac(9,649), rm.get('Robert_Sargos_2001')),
        ep.literature_exp_pair(frac(7,4540), 1 - frac(63,4540), rm.get('robert_2002')),
        ep.literature_exp_pair(frac(1,615), 1 - frac(9,615), rm.get('robert_2002b')),
        ep.literature_exp_pair(frac(1,915), 1 - frac(10,915), rm.get('robert_2002b'))
        ])
...
def add_huxley_exponent_pairs(M):
    for m in range(1, M):
        literature.add_hypothesis(literature_exp_pair(frac(169, 1424 * pow(2, m) - 338), 
...
def add_heath_brown_exponent_pairs(M):
    for m in range(3, M):
        literature.add_hypothesis(literature_exp_pair(frac(2, (m - 1) * (m - 1) * (m + 2)),
...
def A_transform(hypothesis): # van der Corput A transform (weyl-van der Corput inequality)
    pair = hypothesis.data
    return derived_exp_pair(pair.k / (2 * pair.k + 2), frac(1,2) + pair.l/(2 * pair.k + 2),
...
def B_transform(hypothesis): # van der Corput B transform (Poisson summation)
    pair = hypothesis.data
    return derived_exp_pair(pair.l - frac(1,2), pair.k + frac(1,2),
                            f'Follows from "{hypothesis.name}" and taking the van der Corput B transform', 
                            {hypothesis})

def C_transform(hypothesis): # Sargos 2003 transform
    pair = hypothesis.data
    return derived_exp_pair(pair.k / (12 * (1 + 4 * pair.k)), (11*(1+4*pair.k) + pair.l) / (12 * (1+4*pair.k)),
                            f'Follows from "{hypothesis.name}" and taking the Sargos C transform', 
                            {hypothesis})
    
literature.add_hypothesis(Hypothesis(f'van der Corput A transform', 'Exponent pair transform', 
                   Exp_pair_transform(f'van der Corput A transform', A_transform), 'See [van der Corput, 1920]', Reference.make(f'Weyl--van der Corput', 1920)))
literature.add_hypothesis(Hypothesis(f'van der Corput B transform', 'Exponent pair transform',
                   Exp_pair_transform(f'van der Corput B transform', B_transform), 'See [van der Corput, 1920]', Reference.make(f'van der Corput', 1920)))
literature.add_hypothesis(Hypothesis(f'Sargos C transform', 'Exponent pair transform',
                   Exp_pair_transform(f'Sargos C transform', C_transform), 'See [Sargos, 2003]', rm.get('sargos_analog_2003')))
...
def add_literature_bounds_mu():
    literature.add_hypotheses( [
        ...
        literature_bound_mu(frac(1,2), frac(1,6), rm.get('hardy_littlewood_1923')), 
        literature_bound_mu(frac(1,2), frac(193,988), rm.get('walfisz_1924')),
        literature_bound_mu(frac(1,2), frac(27,164), rm.get('titchmarsh_van_1931')),
        literature_bound_mu(frac(1,2), frac(229,1392), rm.get('phillips_zeta_1933')),
        literature_bound_mu(frac(1,2), frac(19,116), rm.get('titchmarsh_order_1942')),
        literature_bound_mu(frac(1,2), frac(15,92), rm.get('min_on_1949')),
        literature_bound_mu(frac(1,2), frac(6,37), rm.get('haneke_verscharfung_1963')),
        literature_bound_mu(frac(1,2), frac(173,1067), Reference.make('Kolesnik', 1973)),
        literature_bound_mu(frac(1,2), frac(35,216), rm.get('kolesnik_order_1982')),
        literature_bound_mu(frac(1,2), frac(139,858), Reference.make('Kolesnik', 1985)),
        literature_bound_mu(frac(1,2), frac(9,56), rm.get('bombieri_order_1986')),
        literature_bound_mu(frac(1,2), frac(89,560), rm.get('watt_exponential_1989')),
        literature_bound_mu(frac(1,2), frac(17,108), rm.get('huxley_exponential_1991')),
        literature_bound_mu(frac(1,2), frac(89,570), rm.get('huxley_exponential_1993')),
        literature_bound_mu(frac(1,2), frac(32,205), rm.get('huxley_exponential_2005')), 
        literature_bound_mu(frac(1,2), frac(13,84), rm.get('bourgain_decoupling_2017')),
    
        # bounds off the critical line
        literature_bound_mu(frac(1934,3655), frac(6299,43860), rm.get('huxley_area_1996')),
        literature_bound_mu(frac(49,51), frac(1,204), rm.get('sargos_analog_2003')),
        literature_bound_mu(frac(361,370), frac(1,370), rm.get('sargos_analog_2003')),
        literature_bound_mu(frac(3,5), frac(1409,12170), rm.get('Lelechenko_linear_2014')),
        literature_bound_mu(frac(4,5), frac(3,71), rm.get('Lelechenko_linear_2014')),
        literature_bound_mu(frac(11,15), frac(1,15), rm.get('demeter_small_2020'))
    ] )
...
literature.add_hypotheses(
    literature_bound_mu(1 - frac(n, pow(2, n) - 2), frac(1, pow(2, n) - 2), Reference.make('van der Corput', 1920))
    for n in range(4, Constants.EXP_PAIR_TRUNCATION))

# Hardy-Littlewood sequence of mu bounds 
literature.add_hypotheses(
    literature_bound_mu(1 - frac(1, pow(2, n - 1)), frac(1, (n + 1) * pow(2, n - 1)), Reference.make('Hardy--Littlewood', 'Unknown date')) # TODO: find the exact year
    for n in range(4, Constants.EXP_PAIR_TRUNCATION))
...
def add_bounds_mu_classic(hypothesis_set):
    hypothesis_set.add_hypothesis(literature.find_hypothesis(data = Bound_mu(frac(1,2), frac(1,6))))
...
literature.add_hypothesis(lv.literature_bound_LV_max([[2, -2, 0], [4, -6, 1]], rm.get('Huxley')))

# Heath-Brown large values theorem: LV(\sigma, \tau) \leq max(2 - 2\sigma, 10 - 13\sigma + \tau)
literature.add_hypothesis(lv.literature_bound_LV_max([[2, -2, 0], [10, -13, 1]], rm.get('heathbrown_zero_1979')))
...
#                           6k - 8k\sigma + \tau)
for k in range(1, 10):
    literature.add_hypothesis(lv.literature_bound_LV_max([
        [2, -2, 0],
        [4 - frac(2, k), -(6 - frac(2, k)), 1],
        [6 * k, -8 * k, 1]
        ], rm.get('jutila_zero_density_1977'), params=f' with k = {k}'))
...
literature.add_hypothesis(zlv.literature_bound_ZLV_Max([[6, -12, 2]], rm.get('heathbrown_twelfth_1978')))
...
zd.add_zero_density(literature, "3/(2 - x)", Itvl(frac(1,2), 1), rm.get('ingham_estimation_1940'))

# Montgomery (1971) Topics in Multiplicative Number Theory
zd.add_zero_density(literature, "1600 * (1 - x) ** (1/2)", Itvl(frac(1,2), 1), Reference.make("Montgomery", 1971))

# M. N. Huxley (1972) On the difference between consecutive primes, Invent. Math., 15, pages 164--170
zd.add_zero_density(literature, "3/(3 * x - 1)", Itvl(frac(1,2), 1), rm.get('Huxley'))

# M. N. Huxley (1973) Large values of Dirichlet polynomials, Acta Arithmetica Volume: 24, pages 329--346
zd.add_zero_density(literature, "39/(115 * x - 75)", Itvl(frac(55,67), frac(189,230)), rm.get('huxley_large_1973'))
zd.add_zero_density(literature, "2", Itvl(frac(189,230), frac(75,89)), rm.get('huxley_large_1973'))
zd.add_zero_density(literature, "48/(37 * (2 * x - 1))", Itvl(frac(61,74), frac(75,89)), rm.get('huxley_large_1973'))

# M. N. Huxley (1975) Large values of Dirichlet polynomials II, Acta Arithmetica Volume: 27, Issue: 1, pages 159--170
zd.add_zero_density(literature, "3/(2 * x)", Itvl(frac(37,42), 1), rm.get('huxley_large_1975a'))
zd.add_zero_density(literature, "48/(37 * (2 * x - 1))", Itvl(frac(61,74), frac(37,42)), rm.get('huxley_large_1975a'))
zd.add_zero_density(literature, "2", Itvl(frac("0.80119"), 1), rm.get('huxley_large_1975a'))
...
zd.add_zero_density(literature, "4/(4 * x - 1)", Itvl(frac(25,28), 1), rm.get('heathbrown_zero_1979'))
zd.add_zero_density(literature, "3/(10 * x - 7)", Itvl(frac(3,4), frac(25,28)), rm.get('heathbrown_zero_1979'))
zd.add_zero_density(literature, "9/(7 * x - 1)", Itvl(frac(11,14), 1), rm.get('heathbrown_zero_1979'))

# A. Ivic (1979) A note on the zero-density estimates for the zeta function, Archiv der Mathematik Volume: 33, pages 155--164
zd.add_zero_density(literature, "6/(5 * x - 1)", Itvl(frac(67,87), 1), rm.get('ivic_note_1979'))
zd.add_zero_density(literature, "3/(34 * x - 25)", Itvl(frac(28,37), frac(74,95)), rm.get('ivic_note_1979'))
zd.add_zero_density(literature, "9/(7 * x - 1)", Itvl(frac(74,95), 1), rm.get('ivic_note_1979'))
zd.add_zero_density(literature, "3/(2 * x)", Itvl(frac(4,5), 1), rm.get('ivic_note_1979'))
zd.add_zero_density(literature, "68/(98 * x - 47)", Itvl(frac(115,166), 1), rm.get('ivic_note_1979'))

# A. Ivic (1980) Exponent pairs and the zeta function of Riemann, Studia Sci. Math. Hung. Volume: 15, pages 157--181
zd.add_zero_density(literature, "3/(2 * x)", Itvl(frac(3831,4791), 1), Reference.make("Ivic", 1980))
zd.add_zero_density(literature, "2", Itvl(frac(11,14), 1), Reference.make("Ivic", 1980))
zd.add_zero_density(literature, "9/(7 * x - 1)", Itvl(frac(41,53), 1), Reference.make("Ivic", 1980))
zd.add_zero_density(literature, "6/(5 * x - 1)", Itvl(frac(13,17), 1), Reference.make("Ivic", 1980))
zd.add_zero_density(literature, "4/(2 * x + 1)", Itvl(frac(17,18), 1), Reference.make("Ivic", 1980))
zd.add_zero_density(literature, "24/(30 * x - 11)", Itvl(frac(155,174), 1), Reference.make("Ivic", 1980))
...
zd.add_zero_density(literature, "(3/2 - x) / (1 - x)", Itvl(frac(155,174), 1), rm.get('jutila_zeroes_1983'))

# A. Ivic (1984) A zero-density theorem for the Riemann zeta-function, Тр. МИАН СССP, том 163, pages 85--89
zd.add_zero_density(literature, "3/(7 * x - 4)", Itvl(frac(3,4), frac(10,13)), rm.get('ivic_zero_1984'))
zd.add_zero_density(literature, "9/(8 * x - 2)", Itvl(frac(10,13), 1), rm.get('ivic_zero_1984'))

# TODO reference 
zd.add_zero_density(literature, "15/(22 * x - 10)", Itvl(frac(10,13), frac(5,6)), Reference.make("Ivic", 1984))
for k in range(2, 100):
    zd.add_zero_density(literature, f"{3*k}/({3*k-2} * x + {2-k})", Itvl(frac(9*k**2 -3*k + 2, 12*k**2 - 5*k + 2), 1), Reference.make("Ivic", 1984))

# B. Conrey (1989)
zd.add_zero_density(literature, "(1 + (1 - 2 * x) * 4/7) / (1 - x)", Itvl(frac(1,2), 1), rm.get('conrey_at_1989'))
...
zd.add_zero_density(literature, "4 / (30 * x - 25)", Itvl(frac(15,16), 1), rm.get('bourgain_remarks_1995'))
zd.add_zero_density(literature, "2 / (7 * x - 5)", Itvl(frac(17,19), 1), rm.get('bourgain_remarks_1995'))
...
zd.add_zero_density(literature, "2", Itvl(frac(25,32), 1), rm.get('bourgain_large_2000'))

# K. Ford (2002) Vinogradov’s integral and bounds for the Riemann zeta function, Proc. London Math. Soc. (3) 85, no. 2, pages 565--633
zd.add_zero_density(literature, "58.05 * (1 - x) ** (1/2)", Itvl(frac(1,2), 1), rm.get('FordZeta'))
...
zd.add_zero_density(literature, "3/(2 * x)", Itvl(frac(3734,4694), 1), Reference.make("Bourgain", 2002))

# D. R. Heath-Brown (2017) Proc. Steklov Inst. Math. 296, no. 1, pages 88--103
zd.add_zero_density(literature, "6.42 * (1 - x) ** (1/2)", Itvl(frac(9,10), 1), rm.get('heathbrown_new_2017'))

#  J. Pintz (2023) Density theorems for Riemann’s zeta-function near the line \Re s = 1. Acta Arithmetica 208.1, pages 1--13
zd.add_zero_density(literature, "2/(15 * x - 12)", Itvl(frac(39,40), frac(41, 42)), rm.get('pintz_density_2023'))
for k in range(4, 100):
    zd.add_zero_density(literature, f"3 / ({k} * (1 - 2 * ({k} - 1) * (1 - x)))", 
                        Itvl(1 - frac(1, 2*k*(k-1)), 1 - frac(1, 2*k*(k+1))), rm.get('pintz_density_2023'))
...
zd.add_zero_density(literature, "24 / (30 * x - 11)", Itvl(frac(279,314), frac(17,18)), rm.get('chen_debruyne_vindas_density_2024'))

# L. Guth, J. Maynard (2024) New large value estimates for Dirichlet polynomials
zd.add_zero_density(literature, "15 / (3 + 5 * x)", Itvl(frac(1,2), 1), rm.get('guth-maynard'))
