...
def run_polytope_edge_tests():
    ...
    for i in range(len(constraints)):
        print(f'face {constraints[i]} intersects with vertices {vertices[incidences[i][0]]}, {vertices[incidences[i][1]]}')
...
def run_polytope_tests():
    ...
    assert set(p_degen.rays) == {(0, 1)} # Ray going to infinity in the direction (0, 1)
    ...
    assert set(Polytope.try_union([p5, p]).get_vertices()) == {(-1, 0), (0, 1), (1, 1), (1, 0), (0, -1)}
