...
def get_trivial_zlv():
    ...
    trivial = Hypothesis('Trivial zeta large value estimate', 'Zeta large value estimate', 
                         lv.Large_Value_Estimate(Piecewise([Affine2([0, 0, 1], domain)])), 
                         'Trivial', Reference.trivial())
    return trivial

def literature_bound_ZLV_Max(bounds, ref, params=''):
    hyp = lv.literature_bound_LV_max(bounds, ref, params)
    return Hypothesis(f'{ref.author()} ({ref.year()}) large value estimate{params}', 'Zeta large value estimate', 
                         hyp.data, 'See [{ref.author(), ref.year()}]', ref)
    
def derived_bound_zeta_LV(bound, proof, deps):
    year = Reference.max_year(tuple(d.reference for d in deps))
    bound = Hypothesis('Derived zeta large value estimate', 'Zeta large value estimate', 
                       lv.Large_Value_Estimate(bound), proof, Reference.derived(year))
...
def best_large_value_estimate(hypotheses, domain):
    ...
    lv_estimates = list(hypotheses.list_hypotheses(hypothesis_type='Zeta large value estimate'))
...
def beta_to_zlv(hypotheses):
    ...
    for be in beta_estimates:
        ...
        tau_upper = Constants.TAU_UPPER_LIMIT if alpha0 == 0 else \
            min(Constants.TAU_UPPER_LIMIT, 1/alpha0)
        tau_lower = Constants.TAU_UPPER_LIMIT if alpha1 == 0 else \
            max(frac(2,1), 1/alpha1)
        ...
        f = Piecewise([
            ...
            Affine2([0, 0, 1], Polytope([
                ...
                [-frac(1,2), 1, 0],     # \sigma > 1/2
                [-tau_upper, 0, 1],     # \tau >= min(some large number, 1/a0)
                [Constants.TAU_UPPER_LIMIT, 0, -1],              # \tau <= TAU_UPPER_LIMIT
                ]))
            ])
        f.simplify()
    ...
    for mb in mu_bounds:
        ...
        f = Piecewise([
            Affine2([0, 0, 0], Polytope([
                ...
                [-frac(1,2), 1, 0],                 # \sigma > 1/2
                [-2, 0, 1],                         # \tau >= 2
                [Constants.TAU_UPPER_LIMIT, 0, -1], # \tau <= min(some large number, 1/a0)
