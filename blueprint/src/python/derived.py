...
def prove_heathbrown_exponent_pairs():
    ...
    hypotheses.add_hypotheses(l for l in literature.list_hypotheses(hypothesis_type='Upper bound on beta')
...
def best_proof_of_exponent_pair(k, l):
    hyp = ep.find_best_proof(frac(3,40), frac(31,40), literature, Proof_Optimization_Method.DATE)
    hyp.recursively_list_proofs()

    hyp = ep.find_best_proof(frac(3,40), frac(31,40), literature, Proof_Optimization_Method.COMPLEXITY)
    hyp.recursively_list_proofs()
...
def prove_ingham_zero_density_estimate():
    ...
    zlv_estimates = literature.list_hypotheses(hypothesis_type='Zeta large value estimate')
    density_estimates = zd.compute_zero_density_estimate(lv_estimates, zlv_estimates, RationalFunction.parse("2 - x"))
