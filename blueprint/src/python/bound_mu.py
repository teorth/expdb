...
def classical_bound_mu(sigma, mu):
    return Hypothesis( f'Classical bound on \\mu({sigma})', 'Upper bound on mu', Bound_mu(sigma,mu), f'Classical', Reference.classical())

# use this constructor to create an upper bound on mu from the literature
def literature_bound_mu(sigma, mu, ref):
    return Hypothesis( f'{ref.author()} bound on \\mu({sigma})', 'Upper bound on mu', Bound_mu(sigma,mu), 
...
def conjecture_bound_mu(sigma, mu, name):
    return Hypothesis(name, 'Upper bound on mu', Bound_mu(sigma,mu), f'Conjecture', Reference.conjectured())
...
def derived_bound_mu(sigma, mu, proof, dependencies):
    ...
    bound = Hypothesis( f'Derived bound on \\mu({sigma})', 'Upper bound on mu', Bound_mu(sigma,mu), proof, Reference.derived(year))
...
def apply_trivial_mu_bound(sigma):
     return derived_bound_mu(sigma, trivial_mu_bound(sigma), f'This is the convexity bound on the Riemann zeta function', set())
...
def apply_functional_equation(bound):
    ...
    return derived_bound_mu(new_sigma, new_mu, f'Follows from "{bound.name}" and the functional equation', {bound})
...
def apply_mu_convexity(b1, b2, theta):
    ...
    return derived_bound_mu(sigma, mu, f'This bound follows from convexity and the bounds \"{b1}\", \"{b2}\"', {b1, b2})

# convert an exponent pair hypothesis to a bound on mu
def obtain_mu_bound_from_exponent_pair(bound):
    return derived_bound_mu(bound.data.l - bound.data.k, bound.data.k, f'This bound follows from the exponent pair ({bound.data.k}, {bound.data.l})', {bound})
...
def best_mu_bound_piecewise(sigma0, sigma1, hypothesis_set):
    ...
    if sigma0 < 0:
        return [("-x + 1/2", sigma0, 0)] + best_mu_bound_piecewise(0, sigma1, hypothesis_set)
    if sigma1 > 1:
    ...
    for i in range(len(verts)):
        ...
        if b1.sigma < b2.sigma and max(sigma0, b1.sigma) <= min(sigma1, b2.sigma):
            mu_bounds.append(Affine((b2.mu - b1.mu) / (b2.sigma - b1.sigma),
...
def prove_mu_bound(sigma, mu, hypothesis_list):
    ...
    if bound.data.mu <= mu:
        ...
        print(f'FAILED to prove \\mu({sigma}) <= {mu}.  The best bound found on \\mu({sigma}) was {bound.data.mu}.')
