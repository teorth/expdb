...
def compute_LV_star(hypotheses, sigma_interval, tau_interval, debug=True, zeta=False):
    ...
    # Iterate through the starting LVERs and "claim" facets in a greedy fashion. 
    # Ignore trivial constraints that can be generated by the bounding box
    deps = [] # Start with a list (not set) to avoid comparisons
    ...
    # There should be no non-trivial constraints left 
    #if len(cons) > 0:
    #    raise RuntimeError(f"{len(cons)} constraints neglected")
