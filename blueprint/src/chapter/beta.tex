...
\begin{lemma}  In Definition \ref{beta-def}, one can take the interval $I$ to be $[N,2N]$.
\end{lemma}

\begin{proof}  Suppose that $\alpha, \overline{\beta}$ are fixed quantities such that the bounds in Definition \ref{beta-def} hold just for $I  = [N,2N]$, thus whenever $T>1$ is unbounded, $N = T^{\alpha+o(1)}$, and $F$ is a model phase function one has
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{python/images/van_der_corput_beta.png}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{python/images/van_der_corput_beta_vs_conjectured.png}
...
\begin{theorem}[1996 Huxley table]\label{huxley-table}  One can bound $\beta(\alpha)$ by
    \begin{align*}
...
\begin{theorem}[Sargos bound]  For any $\alpha > 0$ one has
...
\begin{theorem}[Second Robert bound]\label{beta-R2} If $k \geq 4$ and $\alpha \geq -(1-k\alpha) \frac{k-1}{2k-3}$ then
