...
\begin{proposition}[Automatic uniformity]\label{auto}
\lean{Expdb.automatic_uniformity_of_choicewise_bounded, Expdb.automatic_uniformity_of_choicewise_infinitesimal}
\leanok
Let $E = E_{\ii}$ be a non-empty variable set, and let $f = f_{\ii}: E \to \C$ be a variable function.  \begin{itemize}
...
\begin{remark} The proof gives slightly more as we only need to discard finitely many initial indices rather than pass to a subsequence which may omit infinitely many indices. Therefore, in (i), there is a fixed $C$ such
that $|f_{\ii}(x)|\leq C$ for all sufficiently large $\ii$ and all
$x\in E_{\ii}$.  Similarly, in (ii), for every fixed $\eps>0$, one has
$|f_{\ii}(x)|\leq\eps$ for all sufficiently large $\ii$ and all
$x\in E_{\ii}$.  Conversely, these conclusions clearly imply the corresponding
hypotheses of Proposition \ref{auto}.
\end{remark}
