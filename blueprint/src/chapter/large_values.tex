...
\begin{lemma}[Asymptotic form of large value exponent]\label{lv-asymp} Let $1/2 \leq \sigma \leq 1$, $\tau \geq 0$, and $\rho \geq 0$ be fixed.  Then the following are equivalent:
    \begin{itemize}
...
\begin{lemma}[Basic properties]\label{lv-basic}  
    \begin{itemize}
...
\begin{lemma}[Lower bound]\label{lv-lower}  For any $1/2 < \sigma \leq 1$ and $\tau \geq 0$, one has $\LV(\sigma, \tau) \geq \min(2 - 2 \sigma,\tau)$, while for $\sigma=1/2$ one has $\LV(\sigma,\tau) = \tau$.
\end{lemma}
...
\begin{conjecture}[Montgomery conjecture]\label{montgomery-conj}  One has 
...
\begin{lemma}[Raising to a power]\label{power-lemma}  For any $1/2 \leq \sigma \leq 1$, $\tau \geq 0$, and natural number $k$, one has
...
\begin{theorem}[Montgomery large values theorem]\label{montgomery-lv} If $1/2 \leq \sigma \leq 1$ and $\tau \geq 0$ is such that
...
\begin{corollary}[Converting an exponent pair to a large values theorem]\label{exp-lv}  If $(k,\ell)$ is an exponent pair, and $1/2 \leq \sigma \leq 1$, and $\tau \geq 0$ are fixed, then
...
\begin{proof}  From Lemma \ref{beta-duality} one has
...
\begin{theorem}[Huxley large values theorem]\label{huxley-lv}\cite[Equation~(2.9)]{Huxley} Let $1/2 \leq \sigma \leq 1$ and $\tau \geq 0$ be fixed.  Then one has
...
\begin{theorem}[Heath-Brown large values theorem, preliminary form] Let $1/2 \leq \sigma \leq 1$ and $\tau \geq 0$ be fixed.  If $\LV(\sigma, \tau) \le \rho$ then 
    \[
...
\begin{theorem}[Heath-Brown large values theorem, optimized]\label{hb-opt} Let $1/2 \leq \sigma \leq 1$ and $\tau \geq 0$ be fixed.   One has
...
\begin{lemma}[Second Heath-Brown large values theorem] If $3/4 < \sigma \leq 1$ and $\tau \geq 0$ are fixed, then
...
\begin{theorem}[Jutila large values theorem]\label{jutila-lvt}  For any integer $k \geq 1$, one has
...
\begin{theorem}[Guth--Maynard large values theorem]\label{guth-maynard-lvt} One has
...
\begin{theorem}[Bourgain large values theorem]\label{bourgain-lvt} \cite{bourgain_large_2000} Let $1/2 < \sigma < 1$ and $\tau > 0$, and let $\rho \coloneqq \LV(\sigma,\tau)$.  Let $\alpha_1, \alpha_2 \geq 0$ be real numbers.  Then either
...
\begin{corollary}[Bourgain large values theorem, simplified version]\label{borg-lv-simp} \cite[Lemma 4.60]{bourgain_large_2000} Let the notation be as above, but additionally assume $\tau \leq 3/2$ and $\rho \leq 1$.  Then
...
\begin{theorem}[Ivic large values theorem]\cite[Lemma 8.2]{ivic}  If $\tau \geq 0$ and $1/2 < \sigma < \sigma' < 1$ are fixed, then
