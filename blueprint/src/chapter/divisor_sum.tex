...
\begin{lemma}[$d_1$ exponent]\uses{divisor-def} One has $\alpha_1=0$.
\end{lemma}

\begin{lemma}\cite[Theorem 13.1]{ivic}\uses{divisor-def} One has $\alpha_2 \leq 35/108$.
\end{lemma}

\begin{lemma}\cite{kolesnik}\uses{divisor-def} One has $\alpha_3 \leq 43/96$.
\end{lemma}


\begin{lemma}[Lower bound]\uses{divisor-def} $\alpha_k \geq \frac{1}{2} - \frac{1}{2k}$ for all $k$.
\end{lemma}
...
\begin{lemma}\label{mas}\uses{divisor-def}  Let $k \geq 2$ be an integer. If $M(\sigma,k) = 1$ then $\alpha_k \leq \sigma$.
\end{lemma}
