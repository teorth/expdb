...
\derived 
\code{prove_prime_gap2()}

\begin{proof}
Using the bounds in \Cref{zero_density_estimates_table} and \Cref{zero-density-energy-estimates-table}, in the range $1/2 \le \sigma \le 3/4$ we may take 
\[
A(\sigma) = \frac{30}{13}, \qquad B(\sigma) = \frac{1931 - 2020\sigma}{286(1 - \sigma)}
\]
Applying the definitions in \Cref{gapsquare-from-a} and after a routine computation, one gets $\alpha(\sigma)$, $\beta(\sigma) \le 19/15$ (with room to spare). Next, for $3/4 \le \sigma \le 5/6$ we use 
\[
A(\sigma) = \frac{3}{3\sigma - 1},\qquad B(\sigma) = \frac{526 - 552\sigma}{77(1 - \sigma)}.
\]
For $\sigma \ge 5/6$ we use $B(\sigma) = 12/(4\sigma - 1)$ from \Cref{hb-energy-bound}, along with 
\[
A(\sigma) = \begin{cases}
...
\end{cases}
\]
The first case comes from \Cref{further_ivic_zero} and the second case comes from \Cref{hb-density}, to once again obtain the required bounds (with equality at $\sigma = 7/8$). 
\end{proof}
