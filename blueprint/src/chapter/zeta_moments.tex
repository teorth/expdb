...
\begin{theorem}\label{M_bound_larger_A}\uses{zeta-grow-def}\cite[Theorems 2.1, 2.2]{trudgian-yang}
    We have
    \[
...
\begin{lemma}\label{mad-variant}\uses{mixed-moment-def, lvz-def}  If $1/2 \leq \sigma_0 \leq 1$, $A \geq 1$, and $h \geq 0$ are fixed, then
...
\begin{corollary}\label{ivic-split}\uses{mixed-moment-def, exp-pair-def}  If $(k,\ell)$ is an exponent pair with $k > 0$, then
    $$ M(1/2,6, \geq h) \leq 1$$
and
$$ M(1/2,\frac{2(1+2k+2\ell)}{k}, \leq h) \leq \frac{k+\ell}{k}$$
where
$$ h := \frac{\ell}{2+4l-2k}.$$
\end{corollary}
...
\begin{corollary}\label{ivic-6-large}\cite[(8.56)]{ivic}\uses{mixed-moment-def} $M(1/2, 6, \geq 11/72) \leq 1$ and $M(1/2,24, \leq 11/72) \leq 15/4$.
\end{corollary}

\begin{proof}\uses{zeta-from-exp} Apply Corollary \ref{ivic-split} with the exponent pair $(4/18, 11/18) = BABA(1/6, 2/3)$.
\end{proof}

\begin{lemma}\label{bourgain-remark-1}\cite[Proposition 2]{bourgain_remarks_1995}\uses{lv-def, mixed-moment-def} Suppose that $M(1/2,A,\geq h) \leq 1$ for some $A \geq 4$ and $h \geq 0$.  Then one has
...
\begin{lemma}\label{bourgain-remark-2}\cite[Proposition 5]{bourgain_remarks_1995}\uses{lv-def, zeta-grow-def, mixed-moment-def}  Suppose that $M(1/2,6,\geq h) \leq 1$ for some $h \geq 0$.  Then for any $1/2 \leq \alpha < \sigma < 1$, one has
...
\begin{lemma}[Chen-Debruyne-Vidas large values theorem]\label{cdv-lv}\uses{lv-def, mixed-moment-def}\cite[Lemma A.1]{cdv}  Let $1/2 \leq \sigma \leq 1$ and $\tau \geq \frac{30\sigma-11}{8}$ be fixed. Let $q_0, A_0, q_1, A_1, h$ be fixed quantities such that $M(1/2,q_0, \geq h) \leq A_0$ and $M(1/2,q_0, \leq h) \leq A_1$.  Suppose that $\rho \leq \LV(\sigma,\tau)$ is such that
...
In \cite{cdv-lv} this lemma is applied with $(q_0,A_0) = (6,1)$ and $(q_1,A_1) = (19,3)$ with $h = 2/13$, which follows from Corollary \ref{ivic-split} applied to the exponent pair $(2/7,4/7) = BA(1/6,2/3)$.
