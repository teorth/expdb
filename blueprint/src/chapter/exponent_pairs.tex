...
\begin{theorem}[Sargos $D$-process]\label{sargos_D}\cite[Theorem 7.1]{sargos_points_1995}\uses{exp-pair-def, beta-def}  If $(k,\ell)$ is an exponent pair, then one has
    $$ \beta(\alpha) \leq \max( k_1 + \alpha(\ell_1-k_1), \frac{1}{12} + \frac{2}{3} \alpha)$$
    for all $0 \leq \alpha \leq 1$, where $(k_1,\ell_1) = D(k,\ell)$ is the pair
$$ D(k,\ell) := \left(\frac{5k+\ell+2}{8(5k+3\ell+2)}, \ell_1 := \frac{29k+21\ell+10}{8(5k+3\ell+2)}\right).$$
\end{theorem}
...
{\bf TODO: describe the polytope $H$ that represented the best known exponent pairs from \cite{trudgian-yang}}

The following exponent pair was derived using this database:
