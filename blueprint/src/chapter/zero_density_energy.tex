...
\begin{lemma}[Basic properties of $A^*$]\label{zeroe-basic}\ 
\begin{itemize}
...
The Type II case similarly follows from Lemma \ref{lve-asymp} and \eqref{lve-bound} exactly as in the proof of Lemma \ref{zero-from-large}.
\end{proof}
...
\begin{theorem}[Heath-Brown's additive energy bound]\cite[Theorem 2]{heathbrown_zero_1979}  Let $1/2 \leq \sigma \leq 1$ be fixed.  Then one can bound $A^*(\sigma)$ by
    \begin{align*}
...
\begin{proof} We first suppose that $\sigma \leq 3/4$.  Here we apply Corollary \ref{zeroe-large-cor-0} with $\tau_0 = 2$.  The $\LV^*_\zeta$ supremum is now trivial, so it suffices to show that
...
Write $\tau' \coloneqq \tau/k$. From Lemma \ref{l2-mvt} one has
...
\end{equation}
where $\tau' \coloneqq \tau/k$.  This expression is complicated, so we divide into cases.  
